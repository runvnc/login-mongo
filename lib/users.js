// Generated by ToffeeScript 1.6.3
(function() {
  var Mongolian, db, fs, mustache, nodemailer, opts, passwordHash, randpass, server, smtp, und, users,
    _this = this;

  passwordHash = require('password-hash');

  randpass = require('randpass');

  fs = require('fs');

  nodemailer = require('nodemailer');

  Mongolian = require('mongolian');

  und = require('underscore');

  mustache = require('mustache');

  server = null;

  db = null;

  users = null;

  opts = {
    mail: {
      from: 'root',
      subjectadd: 'User account created',
      bodyadd: "Username: {{name}} password: {{pass}",
      bodyreset: "Username: {{name}} password: {{pass}}"
    }
  };

  smtp = nodemailer.createTransport("Sendmail", "/usr/sbin/sendmail");

  function config(options) {
    und.extend(opts, options);
    if (options.connect != null) {
      db = new Mongolian(options.connect);
    } else {
      db = new Mongolian('mongo://localhost:27017/users');
    }
    if (options.collection != null) {
      return users = db.collection(options.collection);
    } else {
      return users = db.collection('users');
    }
  };

  config({});

  function checkExists(email, cb) {
    var e, existing,
      _this = this;
    users.findOne({
      email: email
    }, function() {
      e = arguments[0], existing = arguments[1];
      return cb(existing != null);
    });
  };

  function addNoEmail(email, name, pass, cb) {
    var _this = this;
    checkExists(email, function(_$$_2) {
      _$cb$_1(!_$$_2);
    });
    function _$cb$_1(_$$_0) {
      if (_$$_0) {
        users.insert({
          email: email,
          name: name,
          passhash: passwordHash.generate(pass)
        });
        return typeof cb === "function" ? cb() : void 0;
      } else {
        return typeof cb === "function" ? cb() : void 0;
      }
    };
  };

  function add(email, name, cb) {
    var e, existing, newuser, options, pass, rendered;
    checkExists(email, function() {
      existing = arguments[0];
      checkExists(email, function(_$$_6) {
        _$cb$_5(!_$$_6);
      });
      function _$cb$_5(_$$_4) {
        if (_$$_4) {
          pass = randpass();
          newuser = {
            email: email,
            name: name,
            passhash: passwordHash.generate(pass)
          };
          users.insert(newuser, function() {
            rendered = mustache.render(opts.bodyadd, newuser);
            try {
              options = {
                from: opts.from,
                to: email,
                subject: opts.subjectadd,
                text: rendered
              };
              smtp.sendMail(options, function(err, res) {
                if (err != null) {
                  return console.log(err);
                } else {
                  return console.log('Message sent: ' + res.message);
                }
              });
            } catch (_error) {
              e = _error;
              console.log('Error sending user mail' + e.message);
              console.log(e);
            }
            return typeof cb === "function" ? cb() : void 0;
          });
        } else {
          return typeof cb === "function" ? cb() : void 0;
        }
      };
    });
  };

  function resetPassword(name) {
    var e, options, pass, rendered, user;
    users.findOne({
      name: name
    }, function() {
      e = arguments[0], user = arguments[1];
      if (user != null) {
        pass = randpass();
        user.passhash = passwordHash.generate(pass);
        users.update({
          name: name
        }, user);
        rendered = mustache.render(opts.bodyreset, user);
        try {
          options = {
            from: opts.from,
            to: users[name].email,
            subject: 'User password reset',
            text: rendered
          };
          return smtp.sendMail(options, function(err, res) {
            if (err != null) {
              return console.log(err);
            } else {
              return console.log('Message sent: ' + res.message);
            }
          });
        } catch (_error) {
          e = _error;
          console.log('Error sending user mail' + e.message);
          return console.log(e);
        }
      }
    });
  };

  function updatePassword(username, oldpass, newpass) {
    if (!checkPassword(username, oldpass)) {
      return false;
    } else {
      users[username].passhash = passwordHash.generate(newpass);
      save();
      return true;
    }
  };

  function checkPassword(username, pass, cb) {
    var e, user;
    users.findOne({
      name: username
    }, function() {
      e = arguments[0], user = arguments[1];
      if (user != null) {
        return passwordHash.verify(pass, user.passhash);
      } else {
        return false;
      }
    });
  };

  exports.config = config;

  exports.add = add;

  exports.checkExists = checkExists;

  exports.resetPassword = resetPassword;

  exports.updatePassword = updatePassword;

  exports.checkPassword = checkPassword;

  exports.addNoEmail = addNoEmail;

  exports.users = users;

}).call(this);
